...
Architecture:
  ...
  - Main right area: tabs for Ephemeris / Trajectories / Sky Map / Planets
  - Bottom status bar: coordinates, visible stars count, UT time
...
class AstroApp:
    ...
    def setup_ui(self):
        ...
        # ── Status bar (packed first = anchored at bottom) ────────────
        ...
        ctk.CTkLabel(p, text="⚙  PARAMETERS", text_color=Config.FG_LABEL,
        ...
        self.btn_geoloc = ctk.CTkButton(p, text="📍", command=self.geolocate,
            fg_color="transparent", border_color=Config.FG_WHITE,
            border_width=1, width=32, font=("Segoe UI", 13))
        self.btn_geoloc.grid(row=3, column=2, padx=(2, 10))
        ...
        sun_fields = [
            ...
            ("DawnAstro", "Astro. Dawn  −18°"),
            ("DawnNaut",  "Naut. Dawn  −12°"),
            ("Dawn",      "Civil Dawn  −6°"),
            ("Rise",      "Sunrise"),
            ("Transit",   "Transit"),
            ("Set",       "Sunset"),
            ("Dusk",      "Civil Dusk  −6°"),
            ("DuskNaut",  "Naut. Dusk −12°"),
            ("DuskAstro", "Astro. Dusk −18°"),
        ]
        ...
        # Left column: planetary ephemeris cards
        ...
        for pname, ptitle, pcolor, pbg in _PLANETS_CONFIG:
        ...
        # Right column: orrery (top-down view of solar system)
    ...
    def _build_data_grid(self, parent, color, fields):
        """
        Creates a grid of label / value rows in a parent frame.

        Each row alternates background color slightly for readability.
        Returns a dict {key: CTkLabel value}.
        """
...
[Continues in part 2...]
    ...
    def _calculate_planets(self, dte, jd, t, lat, lon):
        """Calculates and displays 4 planets ephemeris + updates orrery."""
        if not self.planet_labels:
            return

        # Heliocentric data for orrery
        ...
        for pname, lb in self.planet_labels.items():
            ...
            # Heliocentric longitude for orrery
    ...
    def _plot_orrery(self, t, helio):
        """Draws top-down view of solar system (2D orrery)."""
        ax = self.ax3
        ax.clear()
        ax.set_facecolor(Config.BG_PANEL)
        ax.set_title("Solar System — Top-down View (J2000.0)", color=Config.FG_WHITE,
                     fontsize=10, pad=8)
        ...
        ax.annotate("Sun", xy=(0, 0), xytext=(0.18, 0.05),
        ...
        ax.annotate("Earth", xy=(xe, ye), xytext=(xe + 0.1, ye + 0.1),
        ...
        _PLANET_SIZES = {"Venus": 6, "Mars": 6, "Jupiter": 9, "Saturn": 8}
    ...
    def _update_status(self, lat, lon, dte):
        """Updates status bar (coordinates, stars count, UT time)."""
        self.lbl_status_position.configure(
            text=f"📍 Lat: {lat:+.4f}°  Lon: {lon:+.4f}°")
        n = len(self.visible_stars)
        self.lbl_status_stars.configure(
            text=f"★ {n} star{'s' if n != 1 else ''} visible")
    ...
    def _load_favorites(self):
        """Loads favorites.json. Returns {} if missing."""
    ...
    def _save_favorites(self):
        """Writes self.favorites to favorites.json."""
    ...
    def save_location(self):
        """Prompts for name and saves current coordinates."""
        try:
    ...
    def apply_location(self, choice):
        """Applies selected location coordinates and recalculates."""
        if choice in self.favorites:
            location = self.favorites[choice]
            self.entry_lat.delete(0, tk.END)
            self.entry_lat.insert(0, str(location["lat"]))
            self.entry_lon.delete(0, tk.END)
            self.entry_lon.insert(0, str(location["lon"]))
            self.calculate()
    ...
    def _format_event(self, dt):
        """Formats UT time as local time (offset applied). Returns '--:--' if None."""
        if dt is None:
            return "--:--"
    ...
    def on_hover(self, event):
        """Tooltip on 24h graph (ax1) and polar map (ax2)."""
        c1, c2 = False, False
        ...
        if event.inaxes == self.ax1:
            ...
            if event.xdata is not None and self.visible_stars:
    ...
    def _on_click_map(self, event):
        """Click on sky map — opens detail popup."""
    ...
    def _show_object_detail(self, name, icon, ra, dec, alt, az, mag,
        ...
        """Shows a CTkToplevel popup with details of a celestial object."""
        ...
        if ra is not None:
            infos.append(("Right Ascension", Formatters.hms(ra)))
        if dec is not None:
            infos.append(("Declination", Formatters.dms(dec)))
        infos.append(("Altitude", f"{alt:+.2f}°"))
        infos.append(("Azimuth", f"{az:.2f}°"))
        if mag is not None:
            infos.append(("Magnitude", f"{mag:+.2f}"))
        if alt > 0:
            infos.append(("Visibility", "Above horizon"))
        if extra and 'illum' in extra:
            infos.append(("Illumination", f"{extra['illum']:.1f} %"))
            infos.append(("Phase", Formatters.lunar_phase(
    ...
    def _search_eclipses_gui(self):
        """Launches eclipse search and displays results."""
    ...
    def _search_conjunctions_gui(self):
        ...
        if not results:
            ctk.CTkLabel(self.evt_scroll,
                         text="No events detected over 12 months.",
                         text_color=Config.FG_LABEL).pack(pady=20)
            return
        ...
        for r in results:
            if r['type'] == 'opposition':
                color = Config.FG_GREEN
            else:
                color = Config.FG_PURPLE
            date_str = r['date'].strftime("%d/%m/%Y")
            self._add_evt_line(date_str, r['details'], color)

    def geolocate(self):
        """Detects position via ip-api.com and fills lat/lon."""
        try:
    ...
    def update_live(self):
        """Recursive callback for live mode (self-schedules via root.after)."""
        if self.live_mode:
            now = datetime.utcnow()
            self.entry_date.configure(state="normal")
    ...
    def calculate(self):
        """Reads inputs, calls MeeusEngine and updates entire interface."""
        try:
            s = self.entry_date.get()
            fmt = "%d/%m/%Y %H:%M:%S" if s.count(":") == 2 else "%d/%m/%Y %H:%M"
            dte = datetime.strptime(s, fmt)
            ...
            messagebox.showerror("Error", "Invalid date format or coordinates.")
            return
    ...
    def update_sun_card(self, ra, dec, alt, az, ev, eot=0.0):
        """Updates Sun labels and status badge in header."""
        lb = self.sun_labels
        lb["RA"].configure(text=f"{Formatters.hms(ra)}  ({ra:.2f}h)")
        lb["Dec"].configure(text=f"{Formatters.dms(dec)}  ({dec:.2f}°)")
        ...
        if alt > 0:
            self.lbl_sun_visible.configure(text="🌟 VISIBLE", text_color=Config.FG_GREEN)
        elif alt > -6:
            self.lbl_sun_visible.configure(text="🌅 CIVIL TWILIGHT", text_color=Config.FG_SUN)
        elif alt > -12:
            self.lbl_sun_visible.configure(text="🌃 NAUTICAL TWILIGHT", text_color=Config.BTN_COLOR)
        elif alt > -18:
            self.lbl_sun_visible.configure(text="🌌 ASTRONOMICAL TWILIGHT", text_color=Config.FG_PURPLE)
        else:
            self.lbl_sun_visible.configure(text="🌑 PITCH DARK", text_color=Config.FG_RED)

    def update_moon_card(self, ra, dec, alt, az, illum, phase, ev):
        """Updates Moon labels and status badge in header."""
        lb = self.moon_labels
        lb["RA"].configure(text=f"{Formatters.hms(ra)}  ({ra:.2f}h)")
        lb["Dec"].configure(text=f"{Formatters.dms(dec)}  ({dec:.2f}°)")
        lb["Alt"].configure(text=f"{alt:.2f}°")
        ...
        if alt > 0:
            self.lbl_moon_visible.configure(text="🌟 VISIBLE", text_color=Config.FG_GREEN)
        else:
            self.lbl_moon_visible.configure(text="🌑 BELOW HORIZON", text_color=Config.FG_RED)
    ...
    def plot_graphs(self, dte_ref, lat, lon, s_alt, s_az, m_alt, m_az, redraw_24h):
        """
        Updates the two Matplotlib graphs.

        ax1 (canvas1) : 24h altitude curves — redrawn only if date/location changes.
        ax2 (canvas2) : polar sky map — redrawn at each call.
        """
        # ── 24h Graph ──────────────────────────────────────────────────
        if redraw_24h:
            self.ax1.clear()

            # Recreate annotation (destroyed by clear())
            self.annot = self.ax1.annotate("", xy=(0, 0), xytext=(15, 15),
                textcoords="offset points",
                bbox=dict(boxstyle="round4,pad=0.6", fc=Config.BG_PANEL,
                          ec=Config.FG_LABEL, lw=1),
            ...
            # ── Twilight vertical bands ────────────────────────────────
            # Convert solar event to decimal hour [0–24]
            def _h(key):
                dt = self.sun_events.get(key)
                return None if dt is None else dt.hour + dt.minute / 60.0 + dt.second / 3600.0

            t_aa = _h('dawn_astro');  t_an = _h('dawn_naut')
            t_ac = _h('dawn_civ');    t_lv = _h('rise')
            t_co = _h('set');         t_cc = _h('dusk_civ')
            t_cn = _h('dusk_naut');   t_ca = _h('dusk_astro')

            # Color palette: night → astro twilight → nautical → civil → day
            _C_NIGHT  = "#08081A"   # pitch dark
            _C_ASTRO = "#140F28"   # astronomical twilight
            _C_NAUT  = "#0E1E38"   # nautical twilight
            _C_CIVIL = "#2A1C10"   # civil twilight
            _C_DAY   = "#1E1A0A"   # day (slightly warm background)
        ...
        for name, (ra_h, dec_d, mag) in _STARS.items():
            e_alt, e_az = MeeusEngine.equatorial_to_horizontal(
        ...
        # ── Constellation lines ────────────────────────────────────────
        star_positions = {name: (az_rad, r)
                          for name, _, az_rad, r, _, _ in self.visible_stars}
        for const_name, edges in CONSTELLATIONS.items():
            pts = []
            for ea, eb in edges:
                if ea in star_positions and eb in star_positions:
                    az_a, r_a = star_positions[ea]
                    az_b, r_b = star_positions[eb]
            ...
            if len(pts) >= 4:
                ...
                self.ax2.annotate(const_name, xy=(c_az, c_r),
